...
class UserGear(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='gear_items')
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True)
    category = models.ForeignKey(Category, on_delete=models.SET_NULL, null=True, blank=True)
    weight_grams = models.IntegerField(null=True, blank=True, validators=[MinValueValidator(0)])
...
class Trip(models.Model):
    ...
    # Weather conditions
    expected_temp_min = models.IntegerField(null=True, blank=True, help_text="Temperature in Celsius")
    expected_temp_max = models.IntegerField(null=True, blank=True, help_text="Temperature in Celsius")
    expected_weather = models.CharField(max_length=50, blank=True)
...
class TripGear(models.Model):
    ...
    trip = models.ForeignKey(Trip, on_delete=models.CASCADE, related_name='gear_items')
    gear = models.ForeignKey(UserGear, on_delete=models.CASCADE, related_name='trip_usages')
    
    origin = models.CharField(max_length=20, choices=ORIGIN_CHOICES, default='recommended')
...
class GearUsageStats(models.Model):
    ...
    avg_usefulness_rating = models.DecimalField(max_digits=3, decimal_places=2, null=True, blank=True)
    
    # Context-based usage stored as JSON
    usage_by_activity = models.JSONField(default=dict, blank=True)
    usage_by_weather = models.JSONField(default=dict, blank=True)
    usage_by_duration = models.JSONField(default=dict, blank=True)
...
class GearCatalog(models.Model):
    ...
    photo = models.ImageField(upload_to='catalog_photos/', null=True, blank=True)
