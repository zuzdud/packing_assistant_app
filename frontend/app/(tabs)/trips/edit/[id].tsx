...
export default function EditTripScreen() {
    ...
    useEffect(() => {
        ...
        if (location && start_date && end_date && !weatherFetched && !isLoading) {
    ...
    }, [formData.location, formData.start_date, formData.end_date, isLoading]);
    ...
    const loadData = async () => {
        try {
            const [trip, activitiesData] = await Promise.all([
                tripService.getTrip(Number(id)),
                tripService.getActivities(),
            ]);

            // Check if weather data already exists
            const hasWeatherData = trip.expected_temp_min && trip.expected_temp_max && trip.expected_weather;
            ...
            setFormData({
                ...
                expected_temp_min: trip.expected_temp_min?.toString() || '',
                expected_temp_max: trip.expected_temp_max?.toString() || '',
                expected_weather: Array.isArray(trip.expected_weather) ? trip.expected_weather : [],
            });
            setActivities(trip.activities || []);
            setAvailableActivities(activitiesData);
            setWeatherFetched(!!hasWeatherData);
        } catch (error) {
